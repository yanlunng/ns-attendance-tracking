(function () {
  var searchInput = document.getElementById('vehicle-search-input');
  if (!searchInput) return;

  var roster = window.VEHICLE_SOURCE_ROSTER || [];
  var resultsBox = document.getElementById('vehicle-search-results');
  var hiddenId = document.getElementById('vehicle-selected-roster-id');
  var form = document.getElementById('vehicle-add-form');
  ...
  form.addEventListener('submit', function (e) {
    if (!hiddenId.value) {
      e.preventDefault();
      alert('Pick a driver from the search results first.');
    }
  });
})();
